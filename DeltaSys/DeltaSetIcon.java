/* @Author Sr Mil Games */
public class DeltaSetIcon extends FilesPanelCustomIcon{
    @Override
    public boolean supportFile(File file){
        return file != null && file.getName().endsWith(".delta");
    }
    ...
    public File getIconForFile(File file){
        ...
        return new File(Directories.getProjectFolder()+"/icons/DeltaLogo.png");
    }
        ...
        for (File f : files){
            if (f.isDirectory()){
                ...
                if (f.getName().equalsIgnoreCase("DeltaImageIcons")){
                    File icon = new File(f, "DeltaLogo.png");
